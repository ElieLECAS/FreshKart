...
# Exemple demandé par l'utilisateur
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--date",
        help=(
            "Date cible au format YYYY-MM-DD (par défaut: valeur définie en haut du fichier)"
        ),
        type=str,
    ...
    parser.add_argument(
        "--input-dir",
        help="Dossier d'entrée contenant customers.csv, refunds.csv et orders_YYYY-MM-DD.json",
        type=str,
        default=DEFAULT_INPUT_DIR,
    )
    parser.add_argument(
        "--output-dir",
        help="Dossier de sortie pour CSV et base SQLite",
        type=str,
...
def load_customers(customers_path: Path) -> pd.DataFrame:
    ...
    # Normalisation de colonnes attendues minimalistes
...
def load_refunds(refunds_path: Path) -> pd.DataFrame:
    ...
    # order_id, amount (signe quelconque) -> on convertit en montants négatifs
    df = df.copy()
    # Accepte soit amount_eur soit amount, privilégie amount_eur si présent
...
def load_orders_json(orders_path: Path) -> pd.DataFrame:
    # On s'attend à une structure simple :
    # [
    #   {"order_id": 1, "customer_id": 10, "payment_status": "paid", "channel": "app",
    #    "order_date": "YYYY-MM-DD", "items": [{"sku":"A","quantity":2,"unit_price":3.5}, ...]}
    # ]
    ...
    # Construire order_date à partir de order_date si présent, sinon created_at (date uniquement)
...
def explode_items(orders_df: pd.DataFrame) -> pd.DataFrame:
    # Développe la liste des items en lignes
    ...
    # Renommer pour harmoniser: qty -> quantity si nécessaire
    ...
    # Colonnes minimales attendues pour l'article
...
def apply_business_rules(
    ...
    # 1) Conserver uniquement les commandes payées
    paid_mask = orders_items_df["payment_status"].astype(str).str.lower().eq("paid")
    filtered = orders_items_df[paid_mask].copy()

    # 2) Exclure clients inactifs
    customers_active = customers_df[customers_df["is_active"] == True].copy()  # noqa: E712
    ...
    # 3) Écarter articles avec prix unitaire négatif (consigner rejets)
    ...
    # 4) Déduplication sur order_id (garder première occurrence)
    # Sur le niveau commande: on identifie la première ligne par order_id
...
def write_outputs(
    ...
    # 1.b) Rejets d'articles pour audit (optionnel)
...
def main() -> None:
    ...
    # Utilise toujours la date fournie (par défaut issue des constantes en haut)
